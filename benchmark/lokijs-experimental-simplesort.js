...
(function (root, factory) {
  ...
  return (function () {
    ...
    // experimental version of simplesort which will always attempt array intersect
    // if index is defined on sort property and results are already filtered
    Resultset.prototype.simplesort2 = function (propname, isdesc) {
      if (typeof (isdesc) === 'undefined') {
        isdesc = false;
      }

      // if already filtered, but we want to leverage binary index on sort
      if (this.collection.binaryIndices.hasOwnProperty(propname) && this.filterInitialized) {
        ...
        this.filteredrows = pv.filter(ix);
    ...
    // Experimental version which attempts to dynamically determine 
    //   whether using 'array.sort' or 'array intersect' would be more likely efficent.
    // If there is no binary index on sort property, it will fallback to sort.
    // If the # of total documents / # current results is less than 10 we will use array insersect
    Resultset.prototype.simplesort3 = function (propname, isdesc) {
      if (typeof (isdesc) === 'undefined') {
        isdesc = false;
      }

      var dc = this.collection.data.length;
      var frl = this.filteredrows.length;
      var eff = dc/frl;
      
      // if already filtered, but we want to leverage binary index on sort
      if (eff <= 10 && this.collection.binaryIndices.hasOwnProperty(propname) && this.filterInitialized) {
        var idx, len=this.filteredrows.length, fr=this.filteredrows;
        var io = {};
        // set up hashobject for simple 'inclusion test' with existing (filtered) results
        for(idx=0; idx<len; idx++) {
