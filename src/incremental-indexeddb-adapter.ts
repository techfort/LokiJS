...
(function (root, factory) {
  ...
  return (function () {
    ...
    const DEBUG =
      ...
      !!(window as typeof self & { __loki_incremental_idb_debug: unknown })
    ...
    IncrementalIndexedDBAdapter.prototype._getChunk = function (
      collection: { ensureId: () => void; idIndex: any; data: string | any[] },
      chunkId: number
    ) {
      // 0-99, 100-199, etc.
    ...
    IncrementalIndexedDBAdapter.prototype.saveDatabase = function (
      ...
      try {
        ...
        tx.onerror = function (e: Error) {
          finish(e);
        };

        tx.onabort = function (e: Error) {
          finish(e);
        };

        const store = tx.objectStore("LokiIncrementalData");

        const performSave = function (maxChunkIds?: Record<string, number>) {
          try {
    ...
    function getMaxChunkIds(allKeys: any[]): Record<string, number> {
      const maxChunkIds = {};
    ...
    function lokiChunkVersionId(chunk: { value: string }) {
      try {
        if (chunk) {
    ...
    IncrementalIndexedDBAdapter.prototype._putInChunks = function (
      idbStore: {
        put: (arg0: { key: string; value: any }) => void;
        delete: (arg0: string) => void;
      },
      loki: { collections: any[]; idbVersionId: string },
      incremental: boolean,
      ...
      const prepareCollection = function (
        collection: {
          dirtyIds: any[];
          name: string;
          maxId: number;
        ...
        incremental &&
          collection.dirtyIds.forEach(function (lokiId: number) {
        ...
        const prepareChunk = function (chunkId: string | number) {
    ...
    IncrementalIndexedDBAdapter.prototype.loadDatabase = function (
      ...
      const finish = function (value: any) {
      ...
      this._getAllChunks(
        dbname,
        function (chunks: { length?: any; loki: any; chunkMap: any }) {
          try {
    ...
    function chunksToMap(chunks: any[]) {
      let loki: any;
      const chunkMap = {};
    ...
    function populateLoki(
      loki: { collections: any[] },
      chunkMap: { [x: string]: any },
      deserializeChunk: (arg0: any, arg1: any) => any,
      lazyCollections: string | any[]
    ) {
      loki.collections.forEach(function populateCollection(
        collectionStub: { name: any },
        i: string | number
      ) {
        ...
        if (chunkCollection) {
          ...
          const lokiDeserializeCollectionChunks = function () {
            ...
            dataChunks.forEach(function populateChunk(
              chunk: any,
              i: string | number
            ) {
              if (isLazy) {
              ...
              chunk.forEach(function (doc: any) {
                data.push(doc);
              });
              dataChunks[i] = null;
            });
    ...
    IncrementalIndexedDBAdapter.prototype._getAllChunks = function (
      ...
      function getMegachunks(keys: any) {
        ...
        function processMegachunk(
          ...
          megachunk.forEach(function (chunk: any, i: string | number) {
        ...
        function requestMegachunk(index: number, wave: number) {
      ...
      function getAllChunks() {
        idbReq(
          ...
          function (e: { target: { result: any } }) {
            ...
            allChunks.forEach(function (chunk: any) {
              parseChunk(chunk, deserializeChunk, lazyCollections);
            });
            callback(allChunks);
          },
      ...
      function getAllKeys() {
        function onDidGetKeys(keys: any[]) {
          keys.sort();
          if (keys.length > 100) {
            getMegachunks(keys);
          } else {
    ...
    function parseChunk(
      chunk: { type: string; collectionName: any; value: string },
      deserializeChunk: (arg0: any, arg1: any) => any,
      lazyCollections: string | any[]
    ) {
      classifyChunk(chunk);
    ...
    IncrementalIndexedDBAdapter.prototype.deleteDatabase = function (
      dbname: string,
      callback: (arg0: { success: boolean }) => void
    ) {
      if (this.operationInProgress) {
        throw new Error(
    ...
    function sortChunksInPlace(chunks: any[]) {
      ...
      chunks.sort(function (a: { index: any }, b: { index: any }) {
        return (a.index || 0) - (b.index || 0);
      });
    }

    function createKeyRanges(keys: string | any[], count: number) {
      const countPerRange = Math.floor(keys.length / count);
      const keyRanges = [];
      let minKey: any, maxKey: any;
    ...
    function idbReq(
      request: { onsuccess: (e: any) => any; onerror: any },
      onsuccess: {
        (e: any): void;
        (e: any): void;
        (e: any): void;
