...
(function (root, factory) {
  ...
  return (function () {
     ...
     *
     * @example
     * const idbAdapter = new LokiIndexedAdapter('finance');
     *
     ...
     */
    function LokiIndexedAdapter(appname: string, options: object) {
      this.app = "loki";
     ...
     * // LOAD
     * const idbAdapter = new LokiIndexedAdapter('finance');
     * const db = new loki('test', { adapter: idbAdapter });
     ...
     * @memberof LokiIndexedAdapter
     */
    LokiIndexedAdapter.prototype.loadDatabase = function (
      dbname: string,
     ...
     * @memberof LokiIndexedAdapter
     */
    LokiIndexedAdapter.prototype.saveDatabase = function (
      dbname: string,
      dbstring: string,
      callback?: (_: { success: boolean } | Error) => void
    ) {
      console.log(this);
      const appName = this.app;
      const adapter: {
        ...
        saveDatabase: (
          dbname: string,
          dbstring: string,
          saveCallback?: (_: { success: boolean } | Error) => void
        ) => void;
     ...
     */
    function LokiCatalog(callback: (i: IDBDatabase) => void) {
      this.db = null;
      this.initializeLokiCatalog(callback);
    }

    LokiCatalog.prototype.initializeLokiCatalog = function (
      callback: (i: IDBDatabase) => void
    ) {
      const openRequest = indexedDB.open("LokiCatalog", 1);
      const cat = this;

      // If database doesn't exist yet or its version is lower than our version specified above (2nd param in line above)
      openRequest.onupgradeneeded = function (_e: IDBVersionChangeEvent) {
        // @ts-ignore
        const thisDB = _e.target.result;
      ...
      openRequest.onsuccess = function (_e: Event) {
        // @ts-ignore
        cat.db = _e.target.result;
    ...
    LokiCatalog.prototype.setAppKey = function (
      app,
      key,
      val,
      callback: (_: { success: boolean }) => void
    ) {
